...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    tex.r = tex.r*1.1 + (0.1*burnt.y);
    // reduce the green channel and **SLIGHTLY** dependant on the rotation of the card
    tex.g = tex.g*0.65 + (0.000001*burnt.x);
    // greatly reduce the blue channel
    tex.b = tex.b*0.2;

    tex.rgb = tex.rgb*0.5;
    float contrast_amt = sin(burnt.y*100 + uv.y*4)/cos(burnt.x*50 + uv.x*0.1);
    if (contrast_amt < 0) {
        contrast_amt = -contrast_amt;
    }
    tex.rgb = increaseContrast(tex.rgb, contrast_amt);
    tex.a *= 0.4;
    tex.rgb = increaseContrast(tex.rgb, 2);
    // required
