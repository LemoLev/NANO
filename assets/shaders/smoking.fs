...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    texture_coords += ( 
        sin(smoking.y*7 + (screen_coords.x*40)),
        cos(smoking.y*4 + (screen_coords.y*3))
    )*0.0075;
	vec2 uv = ((((texture_coords)*(image_details)) - texture_details.xy*texture_details.ba)/texture_details.ba);
    vec4 tex = Texel(texture, texture_coords);

    
    number average = (tex.r+tex.b+tex.g)/3.0 + sin(smoking.y*4)*0.1;
    vec4 avg2 = vec4((average+tex.r)/2., (average+tex.g)/2., (average+tex.b)/2., tex.a);
    tex = avg2;
